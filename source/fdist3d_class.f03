...
! update: 04/18/2016
      ...
      public :: fdist3d_003, fdist3d_004, fdist3d_005, fdist3d_006
      public :: fdist3d_007
...
!! HEAVISIDE-WALL Twiss parameter and Piecewise Longitudinal in z (the same particle charge)
      ...
      type, extends(fdist3d) :: fdist3d_006
! Twiss parameter and Cut-gaussian in z (the same particle charge)
         private

         integer :: npx, npy, npz
         real :: qm, sigvz, sigz, zcut
         real :: bcx, bcy, bcz, alphax, alphay, betax, betay
         real :: emitx, emity
         real :: gamma,np
         real, dimension(:), allocatable :: fz, z
         ...
         contains
         procedure, private :: init_fdist3d => init_fdist3d_006
         procedure, private :: dist3d => dist3d_006

      end type fdist3d_006
...
!! With centroid perturbation at sin(kappa (xi-censtart))
         ...
         contains
         procedure, private :: init_fdist3d => init_fdist3d_007
         procedure, private :: dist3d => dist3d_007

      end type fdist3d_007
!
...
!!
      subroutine init_fdist3d_006(this,input,i)
      
         implicit none
         
         class(fdist3d_006), intent(inout) :: this
         type(input_json), intent(inout), pointer :: input
         integer, intent(in) :: i        
! local data
         integer :: npf,npx,npy,npz,npmax
         real :: qm,sigx,sigy,sigz,bcx,bcy,bcz,sigvz,zcut
         real, dimension(2) :: alpha, beta, emit
         real :: gamma,np
         logical :: quiet, evol
         real :: min, max, cwp, n0
         ...
         integer :: indx, indy, indz         
         character(len=20) :: sn,s1
         character(len=18), save :: sname = 'init_fdist3d_006:'
         ...
         call input%get(trim(s1)//'.npmax',npmax)
         call input%get(trim(s1)//'.evolution',evol)
         call input%get(trim(s1)//'.zcut',zcut)
      ...
      end subroutine init_fdist3d_006
!
      subroutine dist3d_006(this,part3d,npp,fd)
...
! edges(4) = upper boundary in z of particle partition
         ...
         logical :: lquiet = .false.
         integer :: idimp, npmax, ierr = 0
         character(len=18), save :: sname = 'dist3d_006:'
         ...
         call PRVDIST32_TWISS_CUT(pt,this%qm,edges,npp, nps, alphax,alphay,   &
         &betax,betay, emitx,emity,sigz,vdx,vdy,vdz,vtz,npx,npy,npz,idimp,&
         &npmax,nx,ny,nz,x0,y0,z0,1,1,4,ierr,vdz,lquiet,zcut)
         ...
         if (ierr /= 0) then
            write (erstr,*) 'PRVDIST32_TWISS_CUT error'
            call this%err%equit(class//sname//erstr)
         endif
...
! PW Twiss with centroid modulation sin(kappa(xi-censtart))
      subroutine init_fdist3d_007(this,input,i)
         ...
         class(fdist3d_007), intent(inout) :: this
         type(input_json), intent(inout), pointer :: input
         integer, intent(in) :: i        
! local data
         integer :: npf,npx,npy,npz,npmax
...
!     END TWISS_L mod
      ...
      end subroutine init_fdist3d_007
!
      subroutine dist3d_007(this,part3d,npp,fd)
